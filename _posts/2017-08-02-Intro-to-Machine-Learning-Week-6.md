...
This may be problematic for the following reason: suppose you train your algorithm and evaluate it on your test case and find that it was able to diagnose 99% of the cases correctly. Error is only 1%. But suppose only 0.5% of the patients in the test sample actually have cancer. A function that always classifies every case as no-cancer will only be wrong for 0.5% of the cases (i.e. those who actually have cancer). This may appear to be an improvement on the previous hypothesis, but it would gravely fail making consistent predictions in the test data. One must be careful when dealing with skewed classification.
